...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="body">
        @if (loading()) {
          <div class="skel-grid">@for (i of [1,2,3,4,5,6]; track i) { <div class="skel-card"><div class="skel-line w-50"></div><div class="skel-line w-30"></div></div> }</div>
        } @else { @if (error()) {
  ...
  styles: [`
    ...
    .err-banner { display: flex; align-items: center; gap: 12px; padding: 14px 20px; background: rgba(196,67,46,0.08); border: 1px solid rgba(196,67,46,0.15); border-radius: var(--radius-md); font-size: var(--text-sm); color: var(--alert-500); }
    .err-banner button { margin-left: auto; padding: 6px 16px; border: 1px solid rgba(196,67,46,0.2); border-radius: var(--radius-sm); background: var(--color-surface); color: var(--alert-500); font-size: var(--text-xs); font-weight: 600; cursor: pointer; }

    .skel-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
    ...
    .action-link:hover { border-color: var(--honey-500); }
