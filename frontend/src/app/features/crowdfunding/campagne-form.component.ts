...
@Component({
      ...
      <div class="form-card">
        <form [formGroup]="form" (ngSubmit)="save()">
          <div class="field">
            ...
            <input id="titre" type="text" formControlName="titre" placeholder="Ex: Projet solidaire" />
          </div>

          <div class="field">
            <label for="description">Description</label>
            <textarea id="description" formControlName="description" rows="4" placeholder="Décrivez votre campagne…"></textarea>
          </div>

          <div class="field">
            <label for="objectif">Objectif (€)</label>
            <input id="objectif" type="number" formControlName="objectif" placeholder="Ex: 5000" />
          </div>
          ...
          <div class="form-actions">
            <button type="button" class="btn-outline" (click)="router.navigate(['/crowdfunding'])">Annuler</button>
            <button type="submit" class="btn-primary" [disabled]="form.invalid || loading()">
              {{ loading() ? 'Enregistrement…' : 'Créer la campagne' }}
            </button>
          </div>
        </form>
      </div>
  ...
  styles: [`
    ...
    .field input, .field textarea, .field select { padding: 10px 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-sm); font-size: 0.88rem; font-family: var(--font-sans); outline: none; }
...
export class CampagneFormComponent {
  ...
  save() {
    ...
    this.http.post('/api/entrepreneuriat/campagnes', {
